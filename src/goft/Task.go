...
func getCronTask() *cron.Cron {
	onceCron.Do(func() {
		logger := cron.VerbosePrintfLogger(log.New(io.MultiWriter(os.Stdout), "cron: ", log.LstdFlags))
		taskCron = cron.New(cron.WithChain(cron.Recover(logger)), cron.WithSeconds())
	})
	return taskCron
}
